...
#		mkdir -p /home/abhijitc/neural_project/results/evo_itr_dump/Closure/temp2/bug"$i"/"$j"
		...
		#exit 0	
		"$d4j_home"/defects4j/framework/bin/run_evosuite.pl -p Chart -v "$i"f -n "$bugid" -o "$basefolder/$bugfolder/$iteration" -c VCMDDU2 -b 20 -t "$basefolder/$evo_temp/$iteration"
